...
if (!is_array($arParams["SECTION_PROPERTY_CODE"])) {
    $arParams["SECTION_PROPERTY_CODE"] = [];
...
if ($this->StartResultCache(false, [($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $arSectionNavParams, $arrSectionFilter])) {
            ...
            if ($arParams['SECTION_PROPERTY_CODE']) {
                foreach ($arParams['SECTION_PROPERTY_CODE'] as $uPropName) {
                    if ($uPropName && $arSection[$uPropName]) {
                        $rsEnum = CUserFieldEnum::GetList([], ["ID" => $arSection[$uPropName]]);
